...
public class AgentScopeWritingAgent implements WritingAgent {
    ...
    private static final String ROUND_REQUEST = """
            ...
            本章正文暂存后，必须调用 commit_chapter 请求用户审核，并在该调用中携带章节记忆、滚动状态和临时计划。审核前不得调用这些状态写入工具；只有用户通过后，commit_chapter 才会写入它们并正式提交章节。目标 Stage 已由当前 Run 上下文绑定，不要自行构造 stage_id，也不要自行宣布提交成功。
            """;
    ...
    private static final String RECOVER_STAGE_REQUEST = """
            ...
            目标 ChapterStage：%s。
            这是一次受限的故障恢复，不接收也不处理新的用户写作要求。先调用 read_staged_chapter，确认候选正文。
            不得调用资料 Agent、原文读取、save_document_summary 或 save_content，不得重写正文。随后调用 commit_chapter 请求用户审核，并在调用中携带章节记忆、滚动状态和临时计划；这些状态仅在用户通过后写入。目标 Stage 由当前 Run 上下文绑定。
            """;
